'''
filename: utils.py
functions: load_data, build_dbs, load_data_into_db, map_city_tier, map_categorical_vars, interactions_mapping 
creator: shivani.agrawal
version: 1
'''
...
def load_data_into_db(db_path, db_file_name, data_directory, lead_scoring_file):
    ...
    leadscoring = load_data( [f"{data_directory}{lead_scoring_file}.csv",])[0]
    leadscoring.reset_index(drop=True)
    leadscoring.to_sql(name='loaded_data', con=cnx, if_exists='replace')
    cnx.close()
    return "Writing to DataBase - loaded_data Done. Check Logs."
...
def map_city_tier(db_path, db_file_name, city_tier_mapping):
    ...
    df.to_sql(name='city_tier_mapped', con=cnx, if_exists='replace')
    cnx.close()
    return "Writing to DataBase-city_tier_mapped Done. Check Logs."
...
def map_categorical_vars(db_path, db_file_name, list_platform, list_medium, list_source):
    ...
    df.to_sql(name='categorical_variables_mapped', con=cnx, if_exists='replace')
    cnx.close()
...
# #############################################################################
def interactions_mapping(db_path, db_file_name, interaction_mapping, index_columns):
    '''
    ...
    df_pivot.drop(columns=['index'], inplace=True, axis=1, errors='ignore')
    cnx.close()
